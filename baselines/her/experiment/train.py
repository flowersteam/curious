...
def launch(env, logdir, n_epochs, num_cpu, seed, replay_strategy, policy_save_interval, clip_return, normalize_obs,
...
@click.command()
@click.option('--env', type=str, default='ModularArm012-v0', help='the name of the OpenAI Gym environment that you want to train on')
...
@click.option('--seed', type=int, default=np.random.randint(int(1e6)), help='the random seed used to seed both the environment and the training code')
@click.option('--policy_save_interval', type=int, default=5, help='the interval with which policy pickles are saved. If set to 0, only the best and latest policy will be pickled.')
@click.option('--replay_strategy', type=click.Choice(['future', 'none']), default='future', help='the HER replay strategy to be used. "future" uses HER, "none" disables HER.')
@click.option('--clip_return', type=int, default=1, help='whether or not returns should be clipped')
@click.option('--normalize_obs', type=bool, default=True, help='whether observations are normalized or not')
